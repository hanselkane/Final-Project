...
##    cv2.imwrite(args['dir']+"trial/"+"Asli"+str(chosen),image)
    ...
    with open(csvpath,'a',newline='') as csvobj:
        ...
        Newlinew=["P2atch"+str(chosen),DataInfo[1],DataInfo[2],"tidur",DataInfo[4],DataInfo[5],DataInfo[6],DataInfo[7]] 
...
##    print("xmin:"+str(DataInfo[4])+"; ymin:"+str(DataInfo[5])+"; xmax:"+str(DataInfo[6])+"ymax:"+str(DataInfo[7]))
    ...
    #random titik tengah patch
    xpatch=random.randint(int(DataInfo[4]),int(DataInfo[6])+1)
    ypatch=random.randint(int(DataInfo[5]),int(DataInfo[7])+1)
    areapatch=((int(DataInfo[6])-int(DataInfo[4]))*(int(DataInfo[7])-int(DataInfo[5])))/3
    hpatch=random.randint(1,(int(DataInfo[7])-int(DataInfo[5])+1))
    wpatch=round(areapatch/hpatch)+1
    value=random.randint(0,254)
    print("Xpatch: ",xpatch,"Ypatch: ",ypatch,"AreaP: ",areapatch,"Hpatch: ",hpatch,"Wpatch: ",wpatch)
    #suka lebih... potong aja
    xmin=round(xpatch-(wpatch/2))
    if(xmin<=1):
        xmin=0
    xmax=round(xpatch+(wpatch/2))
    if(xmax>=159):
        xmax=159
    ymin=round(ypatch-(hpatch/2))
    if(xmax<=1):
        xmax=0
    ymax=round(ypatch+(hpatch/2))
    if(ymax>=120):
        ymax=120
    if((xpatch+(round(wpatch/2)))>=159):
        (xpatch+(round(wpatch/2)))
    for x in range(xmin,xmax):
        for y in range(ymin,ymax):
            image[y,x]=((ironblack[value*3+2]),(ironblack[value*3+1]),(ironblack[value*3+0]))

    cv2.imshow('image',image)
    cv2.imwrite(args['dir']+"trial/"+"P2atch"+str(chosen),image)
